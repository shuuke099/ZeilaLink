  ...
  return (
    ...
    >
      <motion.div
        ...
        className="overflow-hidden rounded-[2.5rem] border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"
      >
        <div className="flex items-center gap-4 border-b border-slate-50 p-8 dark:border-slate-800">
          <div className="h-10 w-10 flex items-center justify-center rounded-xl bg-purple-50 text-purple-600">
            <BookOpen size={20} />
          </div>
          <div>
            <h3 className="text-xl font-black tracking-tight text-[#0b213f] dark:text-white">Vocational Oversight</h3>
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400">Manage training providers and educational certificates.</p>
          </div>
        </div>
      ...
      <section className="grid gap-6 sm:grid-cols-3 mt-8">
        {[
          ...
          <motion.div
            ...
            className="flex flex-col items-center rounded-2xl border border-slate-50 bg-slate-50/30 p-6 text-center dark:border-slate-800 dark:bg-slate-900"
          >
            <stat.icon className={`h-6 w-6 ${stat.color} mb-3`} />
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{stat.label}</p>
            <p className="text-2xl font-black text-[#0b213f] dark:text-white">{stat.val}</p>
          </motion.div>
        ))}
      </section>
    </AdminDashboardPage>
