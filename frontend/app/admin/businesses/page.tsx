...
export default function AdminBusinessesPage() {
  ...
  return <AdminDashboardPage title="Businesses" description="Manage businesses shown in the public directory.">
    <div className="-mt-6 h-[calc(100vh-190px)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
        <div><h1 className="text-xl font-black text-slate-900 dark:text-white">Business Management</h1><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{businesses.length} {businesses.length===1?'business':'businesses'} in the directory</p></div>
        <div className="flex items-center gap-3"><label className="relative hidden sm:block"><Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search businesses" className="h-10 w-56 rounded-lg border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-900 outline-none focus:border-primary dark:border-slate-700 dark:bg-slate-950 dark:text-white"/></label><Link href="/admin/businesses/new" className="inline-flex h-10 items-center gap-2 rounded-lg bg-blue-600 px-4 text-sm font-bold text-white shadow-sm hover:bg-blue-700"><Plus size={18}/>New</Link></div>
      </div>
      ...
      <div className="overflow-auto" style={{maxHeight:'calc(100% - 82px)'}}>
        <table className="min-w-full border-collapse text-left">
          <thead className="sticky top-0 z-10 bg-slate-50 text-xs font-black uppercase tracking-wider text-slate-500 dark:bg-slate-800 dark:text-slate-300"><tr><th className="px-5 py-4">Business</th><th className="px-5 py-4">Category</th><th className="px-5 py-4">Location</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Added</th><th className="px-5 py-4 text-right">Actions</th></tr></thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {loading ? <tr><td colSpan={6} className="px-5 py-16 text-center text-slate-500 dark:text-slate-300">Loading businesses...</td></tr> : filtered.length===0 ? <tr><td colSpan={6} className="px-5 py-16 text-center"><Building2 size={36} className="mx-auto text-slate-300"/><p className="mt-3 font-bold text-slate-700 dark:text-slate-200">{query?'No businesses match your search':'No businesses have been added yet'}</p>{!query&&<Link href="/admin/businesses/new" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-bold text-white"><Plus size={17}/>Add first business</Link>}</td></tr> : filtered.map((business)=><tr key={business.id} className="group hover:bg-blue-50/40 dark:hover:bg-slate-800/70">
              <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-950">{business.logoUrl?<img src={business.logoUrl} alt="" className="h-full w-full object-contain p-1"/>:<Building2 size={20} className="text-blue-600"/>}</div><div className="min-w-0"><div className="flex items-center gap-2"><span className="truncate font-bold text-slate-900 dark:text-white">{business.name}</span>{business.featured&&<Star size={14} className="fill-amber-400 text-amber-400"/>}</div><span className="text-xs text-slate-400">ID: {business.id.slice(0,8)}</span></div></div></td>
              ...
              <td className="px-5 py-4"><div className="flex max-w-xs items-start gap-2 text-sm text-slate-600 dark:text-slate-300"><MapPin size={15} className="mt-0.5 shrink-0 text-slate-400"/><span className="line-clamp-2">{business.address}, {business.city}</span></div></td>
              ...
              <td className="whitespace-nowrap px-5 py-4 text-sm text-slate-500 dark:text-slate-300">{new Intl.DateTimeFormat('en',{dateStyle:'medium'}).format(new Date(business.createdAt))}</td>
