...
import { BookOpen, CalendarDays, ChevronDown, ChevronUp, Clock3, Grid2X2, Heart, List, MapPin, Search, SlidersHorizontal, X } from 'lucide-react';
...
const dateOptions = [['any', 'Any time'], ['week', 'This week'], ['month', 'This month'], ['three', 'Next 3 months'], ['six', 'Next 6 months'], ['custom', 'Choose date']];
...
export default function TrainingsClient({ initialTrainings, loadError = false }: Props) {
  ...
  const dateLabelsSo: Record<string, string> = { any: 'Waqti kasta', week: 'Toddobaadkan', month: 'Bishan', three: '3-da bilood ee soo socda', six: '6-da bilood ee soo socda', custom: 'Dooro taariikh' };
  const localizedDateOptions = dateOptions.map(([value, label]) => [value, language === 'en' ? label : dateLabelsSo[value]]);
  const categories = [{ value: '', label: language === 'en' ? 'All categories' : 'Dhammaan qaybaha' }, ...trainingCategories.map((item) => ({ ...item, label: language === 'en' ? item.label : item.labelSo }))];
  ...
  const visibleTrainings = useMemo(() => {
    ...
    return trainings.filter((item) => {
      ...
      if (date === 'custom' && selectedDate && item.startDate?.slice(0, 10) !== selectedDate) return false;
  ...
  }, [category, date, selectedDate, delivery, location, price, sort, trainings]);

  const resetFilters = () => { setCategory(''); setLocation(''); setPrice('all'); setDate('any'); setSelectedDate(''); setDelivery('all'); setSearch(''); };
  ...
  return <div className="min-h-screen bg-[#fbfbfe] text-slate-900 dark:bg-slate-950 dark:text-slate-100"><Navbar /><main className="pb-16 pt-16">
    ...
    <div className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5 md:grid-cols-[minmax(260px,1fr)_160px_145px_auto]">
        <label className="relative col-span-3 block min-w-0 md:col-span-1"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" size={16} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={language === 'en' ? 'Search courses or programs...' : 'Raadi koorsooyin...'} className={`${fieldClass} w-full pl-10 pr-9 placeholder:text-slate-400 dark:placeholder:text-slate-500`} />{search && <button onClick={() => setSearch('')} aria-label="Clear search" className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-slate-700 dark:text-slate-500 dark:hover:text-slate-200"><X size={15} /></button>}</label>
        <Select value={location} onChange={setLocation} label="Location" options={locations.map((item) => [item, item])} />
        <div className="min-w-0 space-y-2">
          <Select value={date === 'any' ? '' : date} onChange={(value) => setDate(value || 'any')} label={language === 'en' ? 'Date' : 'Taariikhda'} options={localizedDateOptions.slice(1)} />
          {date === 'custom' && <TrainingDatePicker value={selectedDate} onChange={setSelectedDate} language={language} />}
        </div>
        <button onClick={() => setMobileFilters(!mobileFilters)} className={`${fieldClass} flex w-full items-center justify-center gap-1 px-1 text-primary xl:pointer-events-none sm:gap-2 sm:px-4`}><SlidersHorizontal size={13} /> <span className="truncate">Filters</span>{activeFilterCount > 0 && <span className="rounded-full bg-primary px-1.5 py-0.5 text-[8px] text-white">{activeFilterCount}</span>}</button>
      </div>
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] items-center gap-2 sm:w-auto sm:max-w-xl"><Select value={category} onChange={setCategory} label={language === "en" ? "Category" : "Qaybta"} options={categories.slice(1).map((item) => [item.value, item.label])} /><select value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort courses" className="h-10 min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 text-[11px] font-semibold text-slate-600 outline-none focus:border-violet-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"><option value="upcoming">{language === "en" ? "Sort: Upcoming" : "Kala sooc: Soo socda"}</option><option value="price-low">{language === "en" ? "Price: Low to high" : "Qiimaha: Hoose ilaa sare"}</option><option value="price-high">{language === "en" ? "Price: High to low" : "Qiimaha: Sare ilaa hoose"}</option><option value="name">Name: A–Z</option></select><div className="flex overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900"><button onClick={() => setView('grid')} aria-label="Grid view" className={`grid h-9 w-9 place-items-center transition ${view === 'grid' ? 'bg-violet-50 text-violet-700 dark:bg-violet-950/60 dark:text-violet-300' : 'text-slate-400 hover:text-violet-600 dark:text-slate-500 dark:hover:text-violet-300'}`}><Grid2X2 size={15} /></button><button onClick={() => setView('list')} aria-label="List view" className={`grid h-9 w-9 place-items-center border-l border-slate-200 transition dark:border-slate-700 ${view === 'list' ? 'bg-violet-50 text-violet-700 dark:bg-violet-950/60 dark:text-violet-300' : 'text-slate-400 hover:text-violet-600 dark:text-slate-500 dark:hover:text-violet-300'}`}><List size={16} /></button></div></div></div>
      <div className="mt-3 grid items-start gap-4 xl:grid-cols-[220px_minmax(0,1fr)]">
        <aside className={`${mobileFilters ? 'block' : 'hidden'} space-y-3 xl:block`}><FilterSection title="Category"><Select value={category} onChange={setCategory} label={categories[0].label} options={categories.slice(1).map((item) => [item.value, item.label])} /></FilterSection><FilterSection title="Date"><RadioList name="date" value={date} onChange={setDate} options={localizedDateOptions} />{date === 'custom' && <div className="mt-3"><TrainingDatePicker value={selectedDate} onChange={setSelectedDate} language={language} /></div>}</FilterSection><FilterSection title="Location"><RadioList name="location" value={location} onChange={setLocation} options={[['', 'All Locations'], ...locations.slice(0, 5).map((item) => [item, item])]} /></FilterSection><FilterSection title="Format"><RadioList name="delivery" value={delivery} onChange={setDelivery} options={[['all', 'Any format'], ['in_person', 'In person'], ['online', 'Online'], ['hybrid', 'Hybrid']]} /></FilterSection><FilterSection title="Price"><RadioList name="price" value={price} onChange={setPrice} options={[['all', 'Any price'], ['free', 'Free'], ['under100', 'Under $100'], ['100to500', '$100 – $500'], ['over500', 'Over $500']]} /></FilterSection>{activeFilterCount > 0 && <button onClick={resetFilters} className="w-full rounded-lg border border-violet-200 bg-violet-50 py-2.5 text-[11px] font-bold text-violet-700 transition hover:bg-violet-100 dark:border-violet-800 dark:bg-violet-950/50 dark:text-violet-300 dark:hover:bg-violet-950">Clear all filters</button>}</aside>
        <section>{visibleTrainings.length === 0 && !loading ? <div className="rounded-xl border border-dashed border-slate-300 bg-white px-5 py-20 text-center dark:border-slate-700 dark:bg-slate-900"><BookOpen className="mx-auto text-violet-300 dark:text-violet-500" size={40} /><h2 className="mt-4 text-lg font-bold text-slate-900 dark:text-white">No programs found</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Try changing your search or filters.</p><button onClick={resetFilters} className="mt-5 rounded-lg bg-violet-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-violet-700">Reset filters</button></div> : <div className={view === 'grid' ? 'grid grid-cols-2 gap-2.5 sm:gap-3 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5' : 'grid gap-3'}>{visibleTrainings.map((training) => <CourseCard key={training.id} training={training} language={language} list={view === 'list'} />)}</div>}</section>
      </div>
    </div>
  </main></div>;
}

function TrainingDatePicker({ value, onChange, language }: { value: string; onChange: (value: string) => void; language: string }) {
  return <label className="block text-[10px] font-semibold text-slate-600 dark:text-slate-300">
    {language === 'en' ? 'Training start date' : 'Taariikhda bilowga tababarka'}
    <input type="date" value={value} onChange={(event) => onChange(event.target.value)} className={`${fieldClass} mt-1 w-full px-2 dark:[color-scheme:dark]`} />
  </label>;
}

function Select({ value, onChange, label, options }: { value: string; onChange: (value: string) => void; label: string; options: string[][] }) {
  return <ScrollableSelect value={value} onChange={onChange} label={label} options={[{ value: '', label }, ...options.map(([value, label]) => ({ value, label }))]} />;
}
...
export function CourseCard({ training, language, list, className }: { training: Training; language: string; list: boolean; className?: string }) {
