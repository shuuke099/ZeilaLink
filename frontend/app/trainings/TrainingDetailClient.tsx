...
type TrainingDetailClientProps = {
  initialTraining: TrainingDetail;
  publicPath: string;
};
...
export default function TrainingDetailClient({
  ...
  const [enrolled, setEnrolled] = useState(
    Boolean(initialTraining.isEnrolled),
  );
  ...
  const trainingDescription =
    language === 'so' && training.descriptionSo?.trim()
  ...
  const providerDescription =
    language === 'so' && training.provider.descriptionSo?.trim()
      ? training.provider.descriptionSo
      : training.provider.description;

  const enroll = async () => {
    try {
      setError('');
      const response = await api.post(`/trainings/${training.id}/enroll`);
      setEnrolled(true);
  ...
  const handleEnrollClick = () => {
    if (!user) {
      router.push(`/login?redirect=${encodeURIComponent(publicPath)}`);
      return;
    }
  ...
  const learningOutcomes = training.learningOutcomes || parseLearningOutcomes(trainingDescription);
  const requirements = training.requirements || parseRequirements(trainingDescription);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-32 pb-12">
          <div className="space-y-8">
            ...
            <section className="rounded-3xl bg-white p-8 shadow-lg shadow-primary/5">
              <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h1 className="text-3xl font-bold text-primary-darker">{trainingName}</h1>
                ...
                {training.imageUrl && (
                  <div className="rounded-3xl overflow-hidden shadow-soft h-56 sm:h-72 lg:h-80 relative bg-slate-100">
                    ...
                    <img
                      ...
                      alt={`${trainingName} training program`}
                ...
                <div className="bg-white rounded-2xl sm:rounded-3xl p-6 shadow-soft sticky top-24">
                  ...
                  <div className="flex flex-col items-center text-center mb-4 pb-4 border-b border-border">
                    ...
                    <div className="w-full">
                      <p className="font-bold text-lg text-primary-darker mb-1">{providerName}</p>
                      {training.provider?.rating && training.provider.rating > 0 && (
            ...
            {training.descriptionSo && (
              <details className="rounded-3xl border border-border bg-white p-6 shadow-soft">
                <summary className="cursor-pointer font-bold text-primary-darker">
                  {language === 'en' ? 'Akhri af-Soomaali' : 'Read in English'}
                </summary>
                <h2 className="mt-4 text-xl font-black text-primary-darker">
                  {language === 'en' ? training.nameSo || training.name : training.name}
                </h2>
                <p className="mt-3 whitespace-pre-line leading-relaxed text-primary-darker/80">
                  {language === 'en' ? training.descriptionSo : training.description}
                </p>
              </details>
            )}
