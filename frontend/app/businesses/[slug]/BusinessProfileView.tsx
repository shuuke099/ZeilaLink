    ...
    <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
      <div className="min-w-0 space-y-4">
        <section className={`${panel} overflow-hidden`}>
          <div className="relative h-[220px] bg-slate-100 dark:bg-slate-800 sm:h-[330px] lg:h-[370px]">{banner ? <img src={banner} alt={text.name} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-primary"><Building2 size={70}/></div>}{gallery.length > 1 && <span className="absolute bottom-3 right-3 rounded bg-slate-950/75 px-2 py-1 text-[8px] font-bold text-white">1 / {gallery.length}</span>}</div>
      ...
      <aside className="space-y-3 lg:sticky lg:top-20">
        ...
        <section className="rounded-xl border border-violet-100 bg-violet-50 p-4 dark:border-violet-900/60 dark:bg-violet-950/30"><p className="flex gap-2 text-[9px] leading-4 text-slate-600 dark:text-slate-300"><Sparkles size={14} className="shrink-0 text-primary"/>Explore trusted local businesses and services across ZeilaLink.</p><Link href="/businesses" className="mt-3 flex h-9 items-center justify-center rounded-lg bg-primary text-[9px] font-bold text-white">Browse businesses</Link></section>
      </aside>
    </div>
  </main>;
}
