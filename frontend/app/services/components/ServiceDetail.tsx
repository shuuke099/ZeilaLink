...
export default function ServiceDetail({ service, isEn }: ServiceDetailProps) {
  ...
  // Proper listing/provider names stay canonical in every language. Only
  // descriptive content and interface labels are localized.
  ...
  const serviceProcess = isEn
    ? [
        ['Book', 'Choose a date and time that works for you.'],
        ['Inspect', 'We confirm your needs and service details.'],
        ['Service', 'Your professional completes the service.'],
        ['Review', 'We follow up to ensure satisfaction.'],
      ]
    : [
        ['Dalbo', 'Dooro taariikh iyo waqti kugu habboon.'],
        ['Hubin', 'Waxaan xaqiijinnaa baahidaada iyo faahfaahinta adeegga.'],
        ['Adeeg', 'Xirfadlaha ayaa dhammeystiraya adeegga.'],
        ['Qiimee', 'Waxaan kula soconnaa si aan u xaqiijinno qanacsanaantaada.'],
      ];
  ...
  return (
    ...
    {showBookingForm && !service.isDemo && <div className="fixed inset-0 z-[180] grid place-items-center bg-slate-950/70 p-4" onClick={()=>setShowBookingForm(false)}><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-border bg-surface p-5 text-foreground shadow-2xl dark:border-slate-700 dark:bg-slate-900" onClick={event=>event.stopPropagation()}><div className="flex items-center justify-between"><h2 className="text-sm font-extrabold text-heading dark:text-white">{isEn ? 'Request' : 'Dalbo'} {serviceTitle}</h2><button type="button" onClick={()=>setShowBookingForm(false)} className="text-muted hover:text-heading dark:text-slate-400 dark:hover:text-white"><X size={18}/></button></div><div className="mt-4 grid gap-3"><BookingInput label={isEn ? 'Phone number' : 'Lambarka telefoonka'} value={form.customerPhone} onChange={value=>setForm(prev=>({...prev,customerPhone:value}))}/><BookingInput label={isEn ? 'Responsible full name' : 'Magaca buuxa ee masuulka'} value={form.responsibleFullName} onChange={value=>setForm(prev=>({...prev,responsibleFullName:value}))}/><label className="text-[9px] font-semibold text-muted dark:text-slate-300">{isEn ? 'Service date and time' : 'Taariikhda iyo waqtiga adeegga'}<input type="datetime-local" value={form.serviceDateTime} onChange={event=>setForm(prev=>({...prev,serviceDateTime:event.target.value}))} className="mt-1 h-10 w-full rounded-lg border border-border bg-surface px-3 text-[10px] text-heading outline-none focus:border-primary dark:border-slate-700 dark:bg-slate-950 dark:text-white"/></label><BookingInput label={isEn ? 'Location / Address' : 'Goobta / Cinwaanka'} value={form.locationAddress} onChange={value=>setForm(prev=>({...prev,locationAddress:value}))}/><label className="text-[9px] font-semibold text-muted dark:text-slate-300">{isEn ? 'Notes' : 'Qoraal'}<textarea value={form.notes} onChange={event=>setForm(prev=>({...prev,notes:event.target.value}))} className="mt-1 min-h-20 w-full rounded-lg border border-border bg-surface p-3 text-[10px] text-heading outline-none focus:border-primary dark:border-slate-700 dark:bg-slate-950 dark:text-white"/></label><button type="button" onClick={handleBookService} disabled={isBooking} className="h-10 rounded-lg bg-primary text-[10px] font-bold text-white">{isBooking ? (isEn ? 'Redirecting…' : 'Waa lagu gudbinayaa…') : (isEn ? 'Continue to checkout' : 'U gudub lacag-bixinta')}</button>{bookingMessage&&<p className="text-[9px] font-semibold text-primary">{bookingMessage}</p>}</div></div></div>}
...
function DetailPanel({ title, action, children }: { title: string; action?: string; children: ReactNode }) {
  return <section className="rounded-xl border border-border bg-surface p-4 shadow-[0_2px_8px_rgba(15,23,42,.04)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_2px_12px_rgba(0,0,0,.3)] sm:p-5"><div className="mb-3 flex items-center justify-between"><h2 className="text-[12px] font-extrabold text-heading dark:text-white">{title}</h2>{action && <span className="text-[8px] font-semibold text-primary">{action}</span>}</div>{children}</section>;
}

function MiniInfo({ label, value }: { label: string; value: string }) {
  return <div><p className="text-[8px] font-semibold uppercase tracking-wide text-muted dark:text-slate-500">{label}</p><p className="mt-1 truncate font-semibold text-foreground dark:text-slate-200">{value}</p></div>;
}

function BookingInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="text-[9px] font-semibold text-muted dark:text-slate-300">{label}<input value={value} onChange={event=>onChange(event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-border bg-surface px-3 text-[10px] text-heading outline-none focus:border-primary dark:border-slate-700 dark:bg-slate-950 dark:text-white"/></label>;
}
