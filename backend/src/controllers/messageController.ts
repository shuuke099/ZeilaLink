...
const hasMessagingRelationship = async (
  ...
  if (roles.has("worker") && roles.has("employer") && roles.size === 2) {
    ...
    const employerUserId =
      sender.role === "employer" ? sender.id : recipient.id;
  ...
  if (
    ...
    const providerUserId =
      sender.role === "provider" ? sender.id : recipient.id;
...
export const sendMessage = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: "Message must contain 1 to 5000 characters",
      });
    }
...
export const sendContactEmail = async (req: Request, res: Response) => {
  try {
    ...
    if (smsTo) {
      const smsBody = `Contact: ${name} <${email}>${
        subject ? ` | ${subject}` : ""
